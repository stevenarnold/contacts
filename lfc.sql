...
/* In this example , lfc's data ( SANDBOX_SARNOLD.CONTACTS.lfc360FLAT ) is used */
     SELECT count(*)
       FROM SANDBOX_SARNOLD.CONTACTS.lfc360FLAT
;

/* Create workspace table */
/* ( Note name change from dart to lfc from this point forward ) */
CREATE OR REPLACE TABLE lfcWork AS
     SELECT FILENAME
          , CONTACTOWNER
          , CONTACTOWNERALIAS
          , CREATEDBY
...
CREATE OR REPLACE TABLE lfc360_1 AS
...
/* Second Group: Unique Firstname , Lastname , Account */
     CREATE OR REPLACE TABLE lfc360_2 AS
     SELECT sha1(concat(contactID, lastModifiedDate)) AS pk
          , 'lfc' AS provider
          , 2 AS sgroup
          , w.*
...
/* Third Group: Unique Firstname , Lastname , Account not by DataSync */
     CREATE OR REPLACE TABLE lfc360_3 AS
     SELECT sha1(concat(contactID, lastModifiedDate)) AS pk
          , 'lfc' AS provider
          , 3 AS sgroup
          , w.*
...
DELETE
       FROM lfcWork USING lfc360_3 t
      WHERE 1=1
        AND lfcWork.firstname = t.firstname
        AND lfcWork.lastname = t.lastname
        AND lfcWork.accountname = t.accountname
;

/* Fourth Group: Most Recent Unique Firstname , Lastname , Account */
CREATE OR REPLACE TABLE lfc360_4 AS
     SELECT sha1(concat(contactID, lastModifiedDate)) AS pk
          , 'lfc' AS provider
          , 4 AS sgroup
          , w.*
...
CREATE OR replace table lfc360all AS
     SELECT *
       FROM lfc360_1
      UNION ALL
     SELECT *
